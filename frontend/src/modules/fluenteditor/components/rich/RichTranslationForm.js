...
type Props = {|
    ...
    updateTranslation: (Translation) => void,
|};
...
export default function RichTranslationForm(props: Props) {
    ...
    const updateRichTranslation = React.useCallback((value: string, path: MessagePath) => {
    ...
    const getFocusedElement = React.useCallback(() => {
        if (focusedElementIdRef.current && tableBodyRef.current) {
            return tableBodyRef.current.querySelector('textarea#' + focusedElementIdRef.current);
        }
        return null;
    }, []);
    ...
    function handleAccessKeyClick(event: SyntheticMouseEvent<HTMLButtonElement>) {
        if (isReadOnlyEditor) {
            return null;
        }
    ...
    function setFocusedInput(event: SyntheticFocusEvent<HTMLTextAreaElement>) {
        focusedElementIdRef.current = event.currentTarget.id;
    }

    function renderTextarea(value: string, path: MessagePath, maxlength: ?number) {
        return <textarea
            id={ `${path.join('-')}` }
            key={ `${path.join('-')}` }
    ...
    function renderAccessKeys(candidates: Array<string>) {
        ...
        if (id && tableBodyRef.current) {
            const accessKeyElement = tableBodyRef.current.querySelector('textarea#' + id);
            if (accessKeyElement) {
    ...
    function renderAttributes(attributes: ?FluentAttributes, path: MessagePath) {
        if (!attributes) {
            return null;
        }
