...
export default function useSendTranslation() {
    ...
    const nextEntity = useSelector(state => entities.selectors.getNextEntity(state));
    const parameters = useSelector(
        state => navigation.selectors.getNavigationParams(state)
    );
    ...
    return (ignoreWarnings?: boolean, content?: string) => {
        ...
        dispatch(actions.sendTranslation(
            entity,
            translationContent,
            locale,
            pluralForm,
